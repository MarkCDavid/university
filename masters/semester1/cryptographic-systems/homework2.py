...
def weak_collision(function: 'Callable[[bytes, int], Tuple[str, str]]', plaintext: 'str'):
    ...
    digest, digest_short = function(plaintext, 1)
    while True:
        test_plaintext = random_hex_string(8)
        test_digest, test_digest_short = function(test_plaintext.encode("utf-8"), 1)
...
def strong_collision(function: 'Callable[[bytes, int], Tuple[str, str]]'):
    left = random_hex_string(16)
    while True:
        right = random_hex_string(16)
        left_digest, left_digest_short = function(left.encode("utf-8"), 2)
        right_digest, right_digest_short = function(right.encode("utf-8"), 2)
...
def weak_collision_exploitation(hash: 'str', hash_function, plaintext: 'str'):
    ...
    print(f"{A} - {digest}")
    print(f"{collision_text} - {collision_digest}")
