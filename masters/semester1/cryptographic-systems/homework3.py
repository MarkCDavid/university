import random
from etc.prime import factorize, random_prime, prime_totient
...
class Task1:
    def __init__(self):
        ...
        print(f"Diffie-Hellman key exchange finished. Calculated shared key is {self.sharedA} == {self.sharedB}.")
    ...
    def _choose_alpha(self):
        for alpha in random.sample(range(1, self.prime), self.prime - 1):
            print()
            print(f"Picking alpha = {alpha}. Checking if it is a primitive root:")

            gcd = extendedEuclidian(alpha, self.prime).gcd 
            print(f"GCD({alpha}, {self.prime}) = {gcd}.")
            if gcd != 1:
                print(f"{alpha} and {self.prime} are not relatively prime and as such, alpha = {alpha} cannot be a primitive root.")
                continue

            print(f"{alpha} and {self.prime} are relatively prime and as such, alpha = {alpha} could be a primitive root.")
    ...
    def _is_primitive_root(self, alpha):
        for factor in self.factors[:-1]:
            ...
            print(f"Calculating alpha^factor mod p = {alpha}^{factor} mod {self.prime} = {power}")
            if power == 1:
                print(f"As {alpha}^{factor} mod {self.prime} = {power}, alpha = {alpha} is not a primitive root!")
                return False
        print(f"None of the factor powers produced 1, as such alpha = {alpha} is a primitive root!")
        return True

    def _public_component(self, private, name):
        public = powermod(self.alpha, private, self.prime)
        print(f"Calculating alpha^{name} mod p = {self.alpha}^{private} mod {self.prime} = {public}")
        return public
...
class Task2:
    ...
    def _decrypt(self, message, name):
        ...
        ciphertext = powermod(message, self.decryption_key, self.n)
        print(f"C = {name}^e mod n = {message}^{self.decryption_key} mod {self.n} = {ciphertext}.")
        return ciphertext
...
class Task3:
    ...
    def __init__(self, prime, alpha, private, public):
        ...
        print(f"p = {prime}, alpha = {alpha}, private = {private}, public = {public}")
        self.prime = prime
        self.alpha = alpha
    ...
    def _sign(self, message, name):
        ...
        print(f"S1 = alpha^k mod p = {self.alpha}^{self.ephemeral_key} mod {self.prime} = {signature_1}")
...
class Bonus1:
    ...
    def _recover_ephemeral_key(self):
        ...
        if extendedEuclidian(self.task3_signature_2 - self.signature_2, self.prime - 1).gcd != 1:
            print("GCD(Task3_S2 - S2, p - 1) != 1, as such ephemeral key cannot be recovered!")
            return None
        print("GCD(Task3_S2 - S2, p - 1) == 1, as such ephemeral key can be recovered!")
    ...
    def _recover_secret(self):
        ...
        print("Recovery formula derived from https://crypto.stackexchange.com/a/1520")
        print("In addition, just having two ciphertexts that were signed using the same ephemeral key is not always enough.")
        print("As per https://crypto.stackexchange.com/a/1520, we only have a solution if GCD(signature_1, p - 1) = 1, as we are required to calculate inverse of signature_1.")

        if extendedEuclidian(self.public, self.prime - 1).gcd != 1:
            print("GCD(signature_1, p - 1) != 1, as such secret cannot be recovered!")
            return None
        print("GCD(signature_1, p - 1) == 1, as such secret can be recovered!")

        recovered_secret = ((self.M - self.ephemeral_key * self.signature_2) * moduloInverse(self.signature_1, self.prime - 1)) % (self.prime - 1)
        print(f"secret = (message - ephemeral_key * signature_2) * signature_1^(-1) mod (p - 1) = ({self.M} - {self.ephemeral_key} * {self.signature_2}) * ({self.signature_1})^(-1) mod ({self.prime - 1}) = {recovered_secret}")
